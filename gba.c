...
void waitForVBlank(void)
{
    while (SCANLINECOUNTER > 160)
        ; //assure we are not in an incomplete vblank
    while (SCANLINECOUNTER < 160)
        ;            //wait until we are out of vdraw
...
{
    volatile u16 color_ref = color;

    for (int dr = 0; dr < height; dr++) {
        DMA[3].src = &color_ref;
        DMA[3].dst = &videoBuffer[OFFSET(row + dr, col, WIDTH)];
...
{   
    for (int dr = 0; dr < HEIGHT; dr++) {
        int offset = OFFSET(dr, 0, WIDTH);
        DMA[3].src = &image[offset];
...
{
    for (int dr = 0; dr < height; dr++) {
        DMA[3].src = &image[OFFSET(dr, 0, width)];
        DMA[3].dst = &videoBuffer[OFFSET(row + dr, col, WIDTH)];
        DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
    }
}
...
void fillScreenDMA(volatile u16 color)
{
    ...
    for (int dr = 0; dr < HEIGHT; dr++) {
        DMA[3].src = &color_ref;
        DMA[3].dst = &videoBuffer[OFFSET(dr, 0, WIDTH)];
        DMA[3].cnt = WIDTH | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
    }
}
